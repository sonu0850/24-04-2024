...
import {  closeModal, deleteData, editSinglePerson, openModal, popUpEdit } from '../../Store/AuthSlice/authSlice'
...
dispatch(deleteData(deletedata))
    ...
    const editClick=(index)=>{
      console.log("clicker");
      const editdata = tableData.find((item, id)=>id === index)
      console.log("editdaaa+++++", editdata);
      dispatch(editSinglePerson(editdata))
                ...
                <tr key={index} className='text-center'>
                   ...
                   <td> <img src={item.image} alt="Loading" className="h-10 w-10 rounded-full" /></td>
                    ...
                    <Link to={`/View/${index}`}> <button  className='bg-blue-300 px-2 rounded-lg me-1' >View</button></Link>
                        ...
                        <button onClick={()=>editClick(index)} className='bg-green-400 px-2 rounded-lg me-1'>Edit</button>
